...
static MusicPlayResult play_mp3(const char *path, char *status, size_t status_size) {
  mp3dec_ex_t mp3;
  if (mp3dec_ex_open(&mp3, path, MP3D_DO_NOT_SCAN) != 0) {
    set_status(status, status_size, "MP3 open failed");
    return MUSIC_PLAY_OPEN_FAILED;
  }

  int16_t buffer[PCM_SAMPLES_PER_CHUNK];
  ...
  while (1) {
    ...
    const size_t samples = mp3dec_ex_read(&mp3, buffer, PCM_SAMPLES_PER_CHUNK);
    if (samples == 0) {
      break;
    }
    const uint8_t channels = (uint8_t)mp3.info.channels;
    if (channels == 0 || channels > 2 || mp3.info.hz <= 0) {
      result = MUSIC_PLAY_UNSUPPORTED;
      break;
    }
    if (!audio_open) {
      if (!mia_host_audio_open((uint32_t)mp3.info.hz, channels, 16)) {
        result = MUSIC_PLAY_AUDIO_FAILED;
        break;
      }
      audio_open = 1;
    }
    if (mia_host_audio_write_pcm16(buffer, (uint32_t)(samples / channels), channels) < 0) {
      result = MUSIC_PLAY_AUDIO_FAILED;
      break;
    }
  }
