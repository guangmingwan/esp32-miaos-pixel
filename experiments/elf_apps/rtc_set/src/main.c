...
int main(int argc, char *argv[]) {
  ...
  while (!exit_pressed()) {
    uint8_t changed = 0;
    if (pressed(MIA_HOST_BUTTON_UP) && selected_field > 0) {
      --selected_field;
      changed = 1;
    }
    if (pressed(MIA_HOST_BUTTON_DOWN) && selected_field + 1 < 6) {
      ++selected_field;
      changed = 1;
    }
    if (pressed(MIA_HOST_BUTTON_L)) {
      adjust_selected_field(-1);
      changed = 1;
    }
    if (pressed(MIA_HOST_BUTTON_R)) {
      adjust_selected_field(1);
      changed = 1;
    }
