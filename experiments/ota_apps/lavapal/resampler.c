...
#define resampler struct resampler

static int fEqual(float b, float a)
{
...
#define resampler_run(name) int (*name)(resampler *, float **, float *)
...
static int resampler_run_blep_sse(resampler * r, float ** out_, float * out_end)
{
	...
	float * in_ = r->buffer_in + resampler_buffer_size + r->write_pos - r->write_filled;
	int used = 0;
	in_size -= 1;
	if (in_size > 0)
	{
		...
		do
		{
			...
			{
				temp1 = _mm_load_ps((float *)(kernel + i));
				temp1 = _mm_mul_ps(temp1, samplex);
				temp2 = _mm_loadu_ps((float *)out + i * 4);
...
static int resampler_run_cubic_sse(resampler * r, float ** out_, float * out_end)
{
	...
	if (in_size > 0)
	{
		...
		do
		{
			...
			temp1 = _mm_loadu_ps((float *)(in));
			temp2 = _mm_load_ps((float *)(cubic_lut + phase * 4));
...
static int resampler_run_sinc_sse(resampler * r, float ** out_, float * out_end)
{
	...
	if (in_size > 0)
	{
		...
		do
		{
			...
			for (i = 0; i < SINC_WIDTH / 2; ++i)
			{
				temp1 = _mm_loadu_ps((float *)(in + i * 4));
				temp2 = _mm_load_ps((float *)(kernel + i));
				temp1 = _mm_mul_ps(temp1, temp2);
				samplex = _mm_add_ps(samplex, temp1);
			}
