...
--*/
{
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG,
      "[LAVA][MAGICDISPATCH] commit-state actor=%d cmd=%d magic=%d target=%d enemy_hp=%d,%d,%d,%d,%d\n",
      g_Battle.UI.wCurPlayerIndex,
      g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.ActionType,
      g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.wActionID,
      g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.sTarget,
      g_Battle.wMaxEnemyIndex >= 0 ? g_Battle.rgEnemy[0].e.wHealth : 0,
      g_Battle.wMaxEnemyIndex >= 1 ? g_Battle.rgEnemy[1].e.wHealth : 0,
      g_Battle.wMaxEnemyIndex >= 2 ? g_Battle.rgEnemy[2].e.wHealth : 0,
      g_Battle.wMaxEnemyIndex >= 3 ? g_Battle.rgEnemy[3].e.wHealth : 0,
      g_Battle.wMaxEnemyIndex >= 4 ? g_Battle.rgEnemy[4].e.wHealth : 0);
...
--*/
{
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG,
      "[LAVA][MAGICFX] resolve caster=%d obj=%d magic=%d type=%d effect=%d target=%d\n",
      wPlayerIndex,
      wObjectID,
      iMagicNum,
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICRLE] effect=%d size=%d\n", iEffectNum, l);
   if (l <= 0)
   {
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICFX] fire bad chunk obj=%d magic=%d type=%d chunk=%d size=%d\n",
         wObjectID, iMagicNum, gpGlobals->g.lprgMagic[iMagicNum].wType, iEffectNum, l);
      return;
   }
   ...
   if (n <= 0)
   {
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICFX] fire no frames obj=%d magic=%d chunk=%d\n",
         wObjectID, iMagicNum, iEffectNum);
      free(lpSpriteEffect);
      return;
   }

   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICFX] frame-plan obj=%d magic=%d chunk=%d frames=%d total=%d delay=%d target=%d player=%d enemy=%d\n",
      wObjectID, iMagicNum, iEffectNum, n, n,
      gpGlobals->g.lprgMagic[iMagicNum].wFireDelay, sTarget, 1, 0);
   ...
   for (i = 0; i < n; i++)
   {
      ...
      PAL_BattleMakeScene();
      VIDEO_CopyEntireSurface(g_Battle.lpSceneBuf, gpScreen);
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICDRAW] blit-frame cx=%d cy=%d w=%d h=%d frame=%d/%d\n",
         x, y,
         PAL_RLEGetWidth(g_Battle.lpMagicBitmap),
         PAL_RLEGetHeight(g_Battle.lpMagicBitmap),
         i + 1, n);
...
--*/
{
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG,
      "[LAVA][MAGICFX] resolve caster=%d obj=%d magic=%d type=%d effect=%d target=%d summon=%d\n",
      wPlayerIndex,
      wObjectID,
      iMagicNum,
      gpGlobals->g.lprgMagic[iMagicNum].wType,
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICFX] frame-plan obj=%d magic=%d chunk=%d frames=%d loop=%d total=%d delay=%d times=%d shake=%d target=%d player=%d enemy=%d\n",
      wObjectID, iMagicNum, iEffectNum, n,
      gpGlobals->g.lprgMagic[iMagicNum].wFireDelay,
      l,
      gpGlobals->g.lprgMagic[iMagicNum].wSpeed,
      gpGlobals->g.lprgMagic[iMagicNum].wEffectTimes,
      gpGlobals->g.lprgMagic[iMagicNum].wShake,
      sTarget, 1, 0);
   ...
   for (i = 0; i < l; i++)
   {
      ...
      if (l - i > gpGlobals->g.lprgMagic[iMagicNum].wShake)
      {
         ...
         *b = PAL_SpriteGetFrame(lpSpriteEffect, k);
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICFX] frame obj=%d magic=%d chunk=%d step=%d/%d frame=%d delay=%d speed=%d target=%d player=%d enemy=%d has_frame=%d\n",
            wObjectID, iMagicNum, iEffectNum, i + 1, l, k,
            gpGlobals->g.lprgMagic[iMagicNum].wFireDelay,
            gpGlobals->g.lprgMagic[iMagicNum].wSpeed,
            sTarget, 1, 0, *b != NULL ? 1 : 0);
      ...
      else
      {
         ...
         *b = PAL_SpriteGetFrame(lpSpriteEffect, (l - gpGlobals->g.lprgMagic[iMagicNum].wShake - 1) % n);
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICFX] frame obj=%d magic=%d chunk=%d step=%d/%d frame=%d delay=%d speed=%d target=%d player=%d enemy=%d has_frame=%d\n",
            wObjectID, iMagicNum, iEffectNum, i + 1, l,
      ...
      {
         int effectpos[MAX_BATTLE_MAGICSPRITE_ITEMS][2] = {{70, 140}, {100, 110}, {160, 100}};
...
--*/
{
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG,
      "[LAVA][MAGICFX] resolve caster=%d obj=%d magic=%d type=%d effect=%d target=%d enemy=%d\n",
      wEnemyIndex,
      wObjectID,
      iMagicNum,
      gpGlobals->g.lprgMagic[iMagicNum].wType,
      ...
      sTarget,
      1);
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICCAP] enemy=%d obj=%d effect=%d\n",
      wEnemyIndex, wObjectID, iEffectNum);
   ...
   for (i = 0; i < l; i++)
   {
      ...
      {
         int effectpos[MAX_BATTLE_MAGICSPRITE_ITEMS][2] = {{180, 180}, {234, 170}, {270, 146}};
...
--*/
{
...

   {
      int iEnemyDelta = 0;
      int iEnemyHits = 0;
      int iPlayerHPDelta = 0;
      int iPlayerMPDelta = 0;
      ...
      for (i = 0; i <= g_Battle.wMaxEnemyIndex; i++)
      {
         ...
         {
            iEnemyDelta += (SHORT)(g_Battle.rgEnemy[i].wPrevHP - g_Battle.rgEnemy[i].e.wHealth);
            iEnemyHits++;
         }
      }
      for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
      {
         ...
         iPlayerHPDelta += (SHORT)(gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] - g_Battle.rgPlayer[i].wPrevHP);
         iPlayerMPDelta += (SHORT)(gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole] - g_Battle.rgPlayer[i].wPrevMP);
      }

      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICROUND] show actor=%d event=%d obj=%d target=%d enemy_damage=%d enemy_hits=%d player_hp_delta=%d player_mp_delta=%d\n",
         g_Battle.wMovingPlayerIndex,
         g_Battle.UI.wActionType,
         g_Battle.UI.wObjectID,
   ...
   for (i = 0; i < 3; i++)
   {
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICPHASE] step=%d event=%d obj=%d target=%d dist=%d\n",
         i + 1, g_Battle.UI.wActionType, g_Battle.UI.wObjectID,
         g_Battle.UI.iSelectedIndex, dist);
      for (j = 0; j <= g_Battle.wMaxEnemyIndex; j++)
      {
...
--*/
{
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG,
      "[LAVA][BATTLESMOKE] command actor=%d command=%d magic=%d selected=%d\n",
      wPlayerIndex,
      g_Battle.rgPlayer[wPlayerIndex].action.ActionType,
      g_Battle.rgPlayer[wPlayerIndex].action.wActionID,
      sTarget);
   ...
   case kBattleActionAttack:
      ...
      else
      {
         ...
         {
            int division = 1;
            int index[MAX_ENEMIES_IN_TEAM] = {2, 1, 0, 4, 3};
            int x = 1;
   ...
   case kBattleActionMagic:
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG,
         "[LAVA][MAGICCAST] commit actor=%d role=%d command=%d obj=%d target=%d target_player=%d enemy_count=%d\n",
         wPlayerIndex,
         wPlayerRole,
         g_Battle.rgPlayer[wPlayerIndex].action.ActionType,
         wObject,
         ...
         (gpGlobals->g.lprgMagic[wMagicNum].wType == kMagicTypeApplyToPlayer ||
          gpGlobals->g.lprgMagic[wMagicNum].wType == kMagicTypeApplyToParty ||
          gpGlobals->g.lprgMagic[wMagicNum].wType == kMagicTypeTrance) ? 1 : 0,
         g_Battle.wMaxEnemyIndex + 1);
      if (wObject == 0x13B || wObject == 0x145 || wObject == 0x156)
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][FENGSHEN] commit-player actor=%d role=%d cmd=%d selected=%d state_target=%d turn=%d\n",
            wPlayerIndex,
            wPlayerRole,
            g_Battle.rgPlayer[wPlayerIndex].action.ActionType,
            sTarget,
            g_Battle.UI.iSelectedIndex,
            g_Battle.iCurAction);
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][FENGSHEN] commit-magic obj=%d mp_cost=%d role_mp=%d type=%d flags=%d all=%d\n",
            wObject,
            gpGlobals->g.lprgMagic[wMagicNum].wCostMP,
            gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole],
            gpGlobals->g.lprgMagic[wMagicNum].wType,
            gpGlobals->g.rgObject[wObject].magic.wFlags,
            (gpGlobals->g.rgObject[wObject].magic.wFlags & kMagicFlagApplyToAll) ? 1 : 0);
      }
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICDISPATCH] commit-state-result actor=%d result=%d event=%d arg0=%d arg1=%d p_damage=%d hit_count=%d\n",
         wPlayerIndex,
         g_fScriptSuccess ? 1 : 0,
         g_Battle.rgPlayer[wPlayerIndex].action.ActionType,
         sTarget,
         wObject,
         sDamage,
         sTarget == -1 ? g_Battle.wMaxEnemyIndex + 1 : 1);
   ...
   case kBattleActionUseItem:
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ITEMUSE] item_id=%d script_on_use=%d item_flags=0x%04X\n",
         wObject,
         gpGlobals->g.rgObject[wObject].item.wScriptOnUse,
         gpGlobals->g.rgObject[wObject].item.wFlags);

      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ITEMUSE] target_role=%d\n",
         (sTarget == -1) ? 0xFFFF : gpGlobals->rgParty[sTarget].wPlayerRole);
      if (sTarget != -1)
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ITEMUSE] before hp=%d mp=%d role=%d\n",
            gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[sTarget].wPlayerRole],
            gpGlobals->g.PlayerRoles.rgwMP[gpGlobals->rgParty[sTarget].wPlayerRole],
            gpGlobals->rgParty[sTarget].wPlayerRole);
      }
      ...
      if (sTarget != -1)
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ITEMUSE] after hp=%d mp=%d success=%d\n",
            gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[sTarget].wPlayerRole],
            gpGlobals->g.PlayerRoles.rgwMP[gpGlobals->rgParty[sTarget].wPlayerRole],
            g_fScriptSuccess ? 1 : 0);
      }
      ...
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ITEMUSE] apply-to-all done success=%d\n",
            g_fScriptSuccess ? 1 : 0);
      }
      ...
      {
         PAL_AddItemToInventory(wObject, -1);
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ITEMUSE] consumed item=%d remaining=%d\n",
            wObject, PAL_GetItemAmount(wObject));
      }
...
--*/
{
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG,
      "[LAVA][BATTLESMOKE] enemy action=%d target=%d magic=%d hp=%d\n",
      wEnemyIndex,
      sTarget,
      wMagic,
      g_Battle.rgEnemy[wEnemyIndex].e.wHealth);
   ...
   {
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLESMOKE] force enemy magic obj=%d type=%d all=%d\n",
         wMagic,
         gpGlobals->g.rgObject[wMagic].magic.wMagicNumber,
         (gpGlobals->g.rgObject[wMagic].magic.wFlags & kMagicFlagApplyToAll) ? 1 : 0);
      //
      // Magical attack
      //
      if (wMagic == 0xFFFF)
