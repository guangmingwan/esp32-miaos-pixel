				...
				else
				{
					...
					if ((i == 1 || (i >= 6 && i <= 11)) && !g_rcCredits[i])
					{
						...
						if (i == 6 || i == 7)
						{
							...
							{
								char *templates[] = { "${platform}", "${author}", "${year}" };
								char *values[] = { PAL_PLATFORM, PAL_CREDIT, PAL_PORTYEAR };
								int matchlen[] = { 11, 9, 7 };
								int valuelen[] = { sizeof(PAL_PLATFORM) - 1, sizeof(PAL_CREDIT) - 1, sizeof(PAL_PORTYEAR) - 1 };
...
--*/
{
   ...
   else
   {
	   ...
	   //
	   fpMsg = UTIL_OpenRequiredFile(PAL_MESSAGE_FILE);
	   fpWord = UTIL_OpenRequiredFile(PAL_WORD_FILE);
...
)
{
	...
	if (text && text_len > 0)
	{
		...
		for (i = CP_BIG5; i <= CP_GBK; i++)
		{
			...
			int j;
			for (j = invalids = 0; j < length; j++)
			{
				int score = 1;
				int k;
				for (k = 0; k < sizeof(valid_ranges) / sizeof(valid_ranges[0]); k++)
				{
...
{
	va_list ap;
	WCHAR * format_end = format + wcslen(format);
	WCHAR * buffer_end = buffer + count - 1;
	WCHAR chr_buf[2] = { 0, 0 };
