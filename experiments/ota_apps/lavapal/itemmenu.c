...
--*/
{
   ...
   int          iItemTextWidth = 8 * gConfig.dwWordLength + 20;
   int          iLinesPerPage = 7 - gConfig.ScreenLayout.ExtraItemDescLines;
   int          iCursorXOffset = gConfig.dwWordLength * 5 / 2;
   int          iAmountXOffset = gConfig.dwWordLength * 8 + 1;
   int          iPageLineOffset = (iLinesPerPage + 1) / 2;
   int          iPictureYOffset = (gConfig.ScreenLayout.ExtraItemDescLines > 1) ? (gConfig.ScreenLayout.ExtraItemDescLines - 1) * 16 : 0;
   ...
   if (!gConfig.fIsWIN95)
   {
      if (!g_fNoDesc && gpGlobals->lpObjectDesc != NULL)
	  {
         ...
         WCHAR *d = PAL_GetObjectDesc(gpGlobals->lpObjectDesc, wObject);
