...
--*/
{
   ...
   while (TRUE)
   {
      ...
      if (wItemSelected == 0 || wItemSelected == MENUITEM_VALUE_CANCELLED)
      {
         ...
         // Start a new game
         //
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MENU] new game selected\n");
         wItemSelected = 0;
         break;
      }
      else
      {
         ...
         {
            UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MENU] load requested but no save exists\n");
         }
         else
         {
            UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MENU] load slot=%d\n", wItemSelected);
         }
...
--*/
{
   ...
   SDL_Rect  rect = { 195 - dx, 7, 120 + dx, 190 };
   ...
   if (wItemSelected != MENUITEM_VALUE_CANCELLED)
   {
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SYSMENU] slot=%d\n", wItemSelected);
   }
   else
   {
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SYSMENU] close cancel\n");
   }
...
--*/
{
	...
	SDL_Rect  rect = { 130, 100, 125 + max(dx[0] + dx[1], dx[2] + dx[3]), 100 };
...
--*/
{
   ...
   int           nSystemMenuItem = sizeof(rgSystemMenuItem) / sizeof(MENUITEM);

   //
   // Create the menu box.
   //
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SYSMENU] open\n");
   ...
   case 1:
      ...
      if (iSlot != MENUITEM_VALUE_CANCELLED)
      {
         ...
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SYSMENU] save slot=%d\n", iSlot);
      }
      else
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SYSMENU] save failed slot=%d\n", gpGlobals->bCurrentSaveSlot);
      }
      break;
   ...
   case 2:
      ...
      if (iSlot != MENUITEM_VALUE_CANCELLED)
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SYSMENU] load slot=%d\n", iSlot);
         AUDIO_PlayMusic(0, FALSE, 1);
         PAL_FadeOut(1);
      ...
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SYSMENU] load requested but no save exists\n");
      }
      break;
   ...
   case 3:
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SYSMENU] music=%d\n", AUDIO_MusicEnabled());
      break;
   ...
   case 4:
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SYSMENU] sound=%d\n", AUDIO_SoundEnabled());
      break;
   ...
   case 5:
      ...
      // Quit
      //
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SYSMENU] quit\n");
      PAL_QuitGame();
      break;
...
--*/
{
   LPBYTE           buffers = (LPBYTE)malloc(320 * 200 + PAL_RLEBUFSIZE + 50 * 49);
   LPBYTE           bufBackground;
   LPBYTE           bufImage;
   LPBYTE           bufImageBox;
   int              labels0[] = {
   ...
   bufBackground = buffers;
   bufImage = bufBackground + 320 * 200;
   bufImageBox = bufImage + PAL_RLEBUFSIZE;
