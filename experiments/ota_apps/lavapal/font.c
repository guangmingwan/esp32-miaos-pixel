...
#define PAL_LOAD_INTERNAL_FONT(fontdata, height) \
   { \
    ...
    if (!unicode_font) unicode_font = heap_caps_calloc(UNICODE_FONT_SIZE, 32, MALLOC_CAP_SPIRAM);
    if (!font_width)   font_width   = heap_caps_calloc(UNICODE_FONT_SIZE, 1,   MALLOC_CAP_SPIRAM);
