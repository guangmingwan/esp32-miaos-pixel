...
--*/
{
...
#endif
      {
         if (g_KeyMap[i][1] & (kKeyUp | kKeyDown | kKeyLeft | kKeyRight))
         {
            fDirectionDown = TRUE;
         }
   ...
   if (!fDirectionDown)
   {
      memset((void *)g_InputState.dwKeyOrder, 0, sizeof(g_InputState.dwKeyOrder));
...
--*/
{
   ...
   #define FAKE_KEYSYM struct FAKE_KEYSYM
   struct FAKE_KEY {
       FAKE_KEYSYM keysym;
   };
   #define FAKE_KEY struct FAKE_KEY
   struct Fake_Event {
       Uint32 type;
       FAKE_KEY key;
   };
   #define Fake_Event struct Fake_Event
...
--*/
{
   ...
   #define FAKE_TFINGER struct FAKE_TFINGER
   struct Fake_Event {
       Uint32 type;
...
--*/
{
   ...
   PAL_TRACE_LOG("[LAVA][INPUT] clear press=0 dir=%d prev=%d\n",
...
--*/
{
   ...
   DWORD          dwHoldState = 0;
   DWORD          dwInputKey = g_InputState.dwKeyPress;
#endif
   ...
   {
      dwHoldState = g_InputState.dwKeyMaxCount;
   ...
   PAL_TRACE_LOG(
      "[LAVA][INPUT] inkey=%d hold=%d press=%d dir=%d prev=%d\n",
      dwInputKey,
      dwHoldState,
