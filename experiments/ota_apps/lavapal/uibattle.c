...
--*/
{
   ...
   BYTE      rgbStatusColor[kStatusAll] =
   {
      0x5F,  // confused
      0xBF,  // slow
      0x0E,  // sleep
...
--*/
{
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEUI] wait actor=%d enemy_count=%d command=%d\n",
      wPlayerIndex,
...
--*/
{
   ...
   if (wSelectedItem != 0xFFFF)
   {
      if (wSelectedItem != 0)
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEUI] item selected actor=%d obj=%d\n",
            g_Battle.UI.wCurPlayerIndex, wSelectedItem);
...
--*/
{
   ...
   if (wSelectedItem != 0xFFFF)
   {
      if (wSelectedItem != 0)
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEUI] throw selected actor=%d obj=%d\n",
            g_Battle.UI.wCurPlayerIndex, wSelectedItem);
...
//                  g_iCurMiscMenuItem = 0; //disabled due to not same as both original version
            ...
            {
               WORD wPlayerRoleHotkey;

               UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][HOTKEY] input key=%d action=last press=%ld\n",
                  kKeyForce, (long)g_InputState.dwKeyPress);

               wPlayerRoleHotkey = gpGlobals->rgParty[g_Battle.UI.wCurPlayerIndex].wPlayerRole;
               w = PAL_BattleUIPickAutoMagic(wPlayerRoleHotkey, 60);
               UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][HOTKEY] ui action=last actor=%d role=%d mp=%d count=%d sel=%d obj=%d\n",
                  g_Battle.UI.wCurPlayerIndex,
                  wPlayerRoleHotkey,
                  gpGlobals->g.PlayerRoles.rgwMP[wPlayerRoleHotkey],
                  PAL_BattleUICountKnownMagic(wPlayerRoleHotkey),
               ...
               else
               {
                  ...
                  {
                     g_Battle.UI.iSelectedIndex = PAL_BattleSelectAutoTarget();
                     UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][HOTKEY] target enemy obj=%d alive=%d\n",
                        w, g_Battle.wMaxEnemyIndex + 1);
                  }
               }

               if (w != 0)
               {
                  UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][HOTKEY] command accepted action=%s obj=%d ui=%d\n",
                     "last",
               ...
               {
                  UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][HOTKEY] command accepted action=attack obj=0 ui=%d\n",
                     g_Battle.UI.state);
               }
               PAL_BattleCommitAction(FALSE);
            }
         ...
         case kBattleMenuMagicSelect:
            ...
            {
               WORD wLastMagic = PAL_BattleUIPickAutoMagic(wPlayerRole, 60);
               if (wLastMagic != 0)
               {
                  UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][HOTKEY] magic-menu accepted action=last obj=%d\n", wLastMagic);
                  w = wLastMagic;
                }
                else
                {
                  UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][HOTKEY] magic-menu failed action=last\n");
                  w = 0xFFFF;
                }
            }
            else if (g_InputState.dwKeyPress & kKeyUseItem)
            {
               WORD wBestMagic = PAL_BattleUIPickAutoMagic(wPlayerRole, 60);
               if (wBestMagic != 0)
               {
                  UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][HOTKEY] magic-menu accepted action=best obj=%d\n", wBestMagic);
                  w = wBestMagic;
                }
                else
                {
                  UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][HOTKEY] magic-menu failed action=best\n");
                  w = 0xFFFF;
                }
            }
            else
            ...
            if (w != 0xFFFF)
            {
               ...
               if (w != 0)
               {
                  UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICUI] FormatMagicName obj=%d word=%d\n",
                     w, gpGlobals->g.rgObject[w].magic.wMagicNumber);
                  UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEUI] magic name role=%d obj=%d text=%d\n",
                     wPlayerRole, w, gpGlobals->g.rgObject[w].magic.wMagicNumber);
                  g_Battle.UI.wActionType = kBattleActionMagic;
                  g_Battle.UI.wObjectID = w;
...
#endif
                        g_Battle.UI.state = kBattleUISelectTargetPlayer;
                        UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][HOTKEY] target party obj=%d\n", w);
                     }
                  }
...
//         g_Battle.UI.iPrevEnemyTarget = x;  //disabled due to not same as both original version
      ...
      //
       if (s_iFrame & 1)
       {
          ...
          UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEUI] enemy name slot=%d obj=%d text=%d\n",
             i,
