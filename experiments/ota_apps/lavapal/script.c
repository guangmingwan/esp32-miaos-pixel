...
--*/
{
   ...
   case 0x001B:
      ...
      //
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][OP001B] a=%d b=%d object_id=%d delta=%d\n",
         pScript->rgwOperand[0], pScript->rgwOperand[1], wEventObjectID,
         (SHORT)pScript->rgwOperand[1]);
   ...
   case 0x001F:
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][TRIGGER] add item=%d delta=%d total=%d\n",
         pScript->rgwOperand[0], (SHORT)pScript->rgwOperand[1], PAL_CountItem(pScript->rgwOperand[0]));
      break;
   ...
   case 0x0020:
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][TRIGGER] remove item=%d count=%d total=%d\n",
         pScript->rgwOperand[0], x, PAL_CountItem(pScript->rgwOperand[0]));
      if (y <= 0)
      {
         //
   ...
   case 0x0024:
      ...
      if (pScript->rgwOperand[0] != 0)
      {
         ...
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][TRIGGER] state object=%d trigger=%d mode=%d state=%d pos=(%d,%d)\n",
            wCurEventObjectID, pCurrent->wTriggerScript, pCurrent->wTriggerMode,
            pCurrent->sState, pCurrent->x, pCurrent->y);
      }
      break;
   ...
   case 0x0049:
      ...
      if (pScript->rgwOperand[0] != 0)
      {
         ...
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][TRIGGER] set state target=%d value=%d current=%d\n",
            wCurEventObjectID,
            pScript->rgwOperand[1],
            pCurrent->sState);
      }
   ...
   case 0x006D:
      ...
      if (pScript->rgwOperand[0])
      {
         ...
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][TRIGGER] scenehook scene=%d enter=%d teleport=%d\n",
   ...
   case 0x006E:
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][STEP] op=006E view=(%d,%d) party=(%d,%d) delta=(%d,%d) dir=%d frame=%d layer=%d\n",
         PAL_X(gpGlobals->viewport), PAL_Y(gpGlobals->viewport),
         PAL_X(gpGlobals->partyoffset), PAL_Y(gpGlobals->partyoffset),
         (SHORT)pScript->rgwOperand[0], (SHORT)pScript->rgwOperand[1],
         gpGlobals->wPartyDirection,
         gpGlobals->g.PlayerRoles.rgwWalkFrames[gpGlobals->rgParty[0].wPlayerRole],
         gpGlobals->wLayer);
      break;
...
--*/
{
   ...
   while (wScriptEntry != 0 && !fEnded)
   {
      ...
      PAL_TRACE_LOG("[LAVA][TRIGGERSTEP] object=%d idx=%d op=%d a=%d b=%d c=%d\n",
         wEventObjectID,
         wScriptEntry,
         pScript->wOperation, pScript->rgwOperand[0],
         pScript->rgwOperand[1], pScript->rgwOperand[2]);
      PAL_TRACE_LOG("[LAVA][SCRIPT] idx=%d op=%d a=%d b=%d c=%d\n",
         wScriptEntry,
         pScript->wOperation, pScript->rgwOperand[0],
         pScript->rgwOperand[1], pScript->rgwOperand[2]);
      ...
      case 0x0007:
         ...
         //
         {
            WORD wBattleNextEntry;

         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLE] script-start team=%d lose=%d flee=%d\n",
            pScript->rgwOperand[0], pScript->rgwOperand[1], pScript->rgwOperand[2]);
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEOP] object=%d idx=%d team=%d win_jump=%d flee_jump=%d boss=%d\n",
            wEventObjectID,
            wScriptEntry,
            pScript->rgwOperand[0],
            pScript->rgwOperand[1],
            pScript->rgwOperand[2],
            pScript->rgwOperand[2] == 0 ? 1 : 0);
         i = PAL_StartBattle(pScript->rgwOperand[0], !pScript->rgwOperand[2]);

         if (i == kBattleResultLost && pScript->rgwOperand[1] != 0)
         {
            wBattleNextEntry = pScript->rgwOperand[1];
         }
         else if (i == kBattleResultFleed && pScript->rgwOperand[2] != 0)
         {
            wBattleNextEntry = pScript->rgwOperand[2];
         }
         else
         {
            wBattleNextEntry = wScriptEntry + 1;
         }
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEOP] result=%d next=%d\n", i, wBattleNextEntry);
         wScriptEntry = wBattleNextEntry;
      ...
      case 0xFFFF:
         ...
         if (gConfig.pszMsgFile)
         {
            ...
            while ((iMsg = PAL_GetMsgNum(pScript->rgwOperand[0], msgSpan, idx++)) >= 0)
               ...
               {
                  UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][DIALOGPAGE] msg=%d page=%d\n", iMsg, idx - 1);
                  UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][DIALOGTEXT] msg=%d\n", iMsg);
         ...
         {
            UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][TRIGGERMSG] object=%d idx=%d msg=%d src=%s\n",
               wEventObjectID,
               wScriptEntry,
               pScript->rgwOperand[0],
               gConfig.pszMsgFile ? "mmsg" : "fallback");
            UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][DIALOG] object=%d idx=%d\n", wEventObjectID, wScriptEntry);
      ...
      default:
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][TRIGGER] idx=%d op=%d\n",
            wScriptEntry, pScript->wOperation);
         PAL_ClearDialog(TRUE);
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][TRIGGER] object=%d next=%d success=%d\n",
      wEventObjectID, wNextScriptEntry, g_fScriptSuccess ? 1 : 0);
   if (wEventObjectID > 0 && wEventObjectID <= MAX_EVENT_OBJECTS)
   {
      LPEVENTOBJECT pLogEvtObj = &(gpGlobals->g.lprgEventObject[wEventObjectID - 1]);
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][TRIGGER] state object=%d trigger=%d mode=%d state=%d pos=(%d,%d)\n",
         wEventObjectID,
         pLogEvtObj->wTriggerScript,
         pLogEvtObj->wTriggerMode,
         pLogEvtObj->sState,
         pLogEvtObj->x,
         pLogEvtObj->y);
   }
...
begin:
   ...
   PAL_TRACE_LOG("[LAVA][AUTOSTEP] obj=%d idx=%d op=%d a=%d b=%d c=%d\n",
        wEventObjectID, wScriptEntry,
        pScript->wOperation, pScript->rgwOperand[0],
       pScript->rgwOperand[1], pScript->rgwOperand[2]);
