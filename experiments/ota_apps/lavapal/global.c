...
)
{
	...
	for (i = 0; i < sizeof(fps) / sizeof(FILE *); i++)
	{
		//
...
)
{
	...
	if (NULL != (fp = UTIL_OpenFile(filename)))
	{
		...
		for (ptr = word_buf; ptr < word_buf + word_len; ptr++)
		{
			if (!*ptr)
				*ptr = ' ';
		}
...
--*/
{
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][INIT] fIsWIN95=%d\n", gConfig.fIsWIN95 ? 1 : 0);
   ...
   //
   if (!gConfig.pszMsgFile) PAL_SetCodePage(PAL_DetectCodePage(PAL_WORD_FILE));
   ...
   gpGlobals->lpObjectDesc = gConfig.fIsWIN95 ? NULL : PAL_LoadObjectDesc(PAL_OBJECT_DESC_FILE);
   gpGlobals->bCurrentSaveSlot = 1;

   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][INIT] desc=%d save_slot=%d avi=%d\n",
      gpGlobals->lpObjectDesc != NULL ? 1 : 0,
      gpGlobals->bCurrentSaveSlot,
      gConfig.fEnableAviPlay ? 1 : 0);
...
#define SAVEDGAME_COMMON struct tagSAVEDGAME_COMMON
#define LPSAVEDGAME_COMMON struct tagSAVEDGAME_COMMON *

struct tagSAVEDGAME_DOS
{
	WORD             wSavedTimes;             // saved times
	WORD             wViewportX, wViewportY;  // viewport location
	WORD             nPartyMember;            // number of members in party
...
#define SAVEDGAME_DOS struct tagSAVEDGAME_DOS
#define LPSAVEDGAME_DOS struct tagSAVEDGAME_DOS *

struct tagSAVEDGAME_WIN
{
	WORD             wSavedTimes;             // saved times
	WORD             wViewportX, wViewportY;  // viewport location
	WORD             nPartyMember;            // number of members in party
...
#define SAVEDGAME_WIN struct tagSAVEDGAME_WIN
#define LPSAVEDGAME_WIN struct tagSAVEDGAME_WIN *
...
)
{
	...
	UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][LOADRAW] slot=%d bytes=%d follower=%d\n",
	   iSaveSlot,
	   (int)size,
	   gpGlobals->nFollower);

	UTIL_LogOutput(LOGLEVEL_DEBUG,
	   "[LAVA][LOAD] slot=%d scene=%d view=(%d,%d) party=(%d,%d) dir=%d count=%d\n",
	   iSaveSlot,
	   gpGlobals->wNumScene,
	   PAL_X(gpGlobals->viewport), PAL_Y(gpGlobals->viewport),
	   PAL_X(gpGlobals->partyoffset), PAL_Y(gpGlobals->partyoffset),
	   gpGlobals->wPartyDirection,
	   gpGlobals->wMaxPartyMemberIndex + 1);
	UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][LOAD] visible_objects=%d range=%d..%d\n",
	   gpGlobals->g.rgScene[gpGlobals->wNumScene].wEventObjectIndex - gpGlobals->g.rgScene[gpGlobals->wNumScene - 1].wEventObjectIndex,
...
)
{
	...
	UTIL_LogOutput(LOGLEVEL_DEBUG,
	   "[LAVA][SAVE] slot=%d scene=%d view=(%d,%d) dir=%d count=%d saved=%d\n",
	   iSaveSlot,
	   gpGlobals->wNumScene,
	   PAL_X(gpGlobals->viewport), PAL_Y(gpGlobals->viewport),
	   gpGlobals->wPartyDirection,
...
{
	UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SAVE] begin slot=%d saved=%d\n", iSaveSlot, wSavedTimes);
	if (gConfig.fIsWIN95)
...
--*/
{
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][LOAD] begin slot=%d\n", iSaveSlot);
   ...
   if (iSaveSlot == 0 || PAL_LoadGame(iSaveSlot) != 0)
   {
      ...
      if (iSaveSlot == 0)
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][NEWGAME] slot=0 default game loaded\n");
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][NEWGAME] scene=%d map=%d enter=%d teleport=%d\n",
            1,
            gpGlobals->g.rgScene[0].wMapNum,
            gpGlobals->g.rgScene[0].wScriptOnEnter,
            gpGlobals->g.rgScene[0].wScriptOnTeleport);
      }
      else
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][LOAD] slot=%d load failed, fallback newgame\n", iSaveSlot);
      }
   }
...
{
   BOOL           fSuccess = FALSE;
   BOOL           fHPChanged = FALSE;
   BOOL           fMPChanged = FALSE;
   WORD           wOrigHP = gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole];
   WORD           wOrigMP = gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole];

   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ADJUST_HP] role=%d hp=%d max_hp=%d delta=%d\n",
      wPlayerRole,
      wOrigHP,
      gpGlobals->g.PlayerRoles.rgwMaxHP[wPlayerRole],
      sHP);
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ADJUST_MP] role=%d mp=%d max_mp=%d delta=%d\n",
      wPlayerRole,
      wOrigMP,
   ...
   if (gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole] > 0)
   {
      ...
      fHPChanged = (wOrigHP != gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole]);
      fMPChanged = (wOrigMP != gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole]);
   }

   if (fHPChanged)
   {
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ADJUST_HP] wrote hp=%d\n",
         gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole]);
   }
   else
   {
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ADJUST_HP] no change hp=%d next=%d\n",
         wOrigHP, gpGlobals->g.PlayerRoles.rgwHP[wPlayerRole]);
   }

   if (fMPChanged)
   {
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ADJUST_MP] wrote mp=%d\n",
         gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole]);
   }
   else
   {
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ADJUST_MP] no change mp=%d next=%d\n",
         wOrigMP, gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole]);
   }
...
--*/
{
   ...
   for (i = 0; i < sizeof(PLAYERROLES) / sizeof(WORD[MAX_PLAYER_ROLES]); i++)
   {
