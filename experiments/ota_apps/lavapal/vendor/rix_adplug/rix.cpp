...
void CrixPlayer::rewind(int subsong)
{
  ...
  {
    // SDLPAL and the game scripts use the raw MKF offset-table index.
    // Do not collapse duplicate offsets: that changes the meaning of every
    // RIX number after the empty/padding entries in MUS.MKF.
    uint32_t offset, next;
    const uint32_t table_end = RIX_GET32(file_buffer, 0) / 4;
    if ((uint32_t)subsong + 1 >= table_end)
      subsong = (int)table_end - 2;
    offset = RIX_GET32(file_buffer, subsong);
    next = RIX_GET32(file_buffer, subsong + 1);
    if (offset > pos) offset = pos;
    if (next > pos || next < offset) next = pos;
