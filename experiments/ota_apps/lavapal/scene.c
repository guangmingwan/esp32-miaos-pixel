...
#define SPRITE_TO_DRAW struct tagSPRITE_TO_DRAW
...
--*/
{
   ...
   int       sx = PAL_X(gpGlobals->viewport) + PAL_X(lpSpriteToDraw->pos) - lpSpriteToDraw->iLayer/2;
   int       sy = PAL_Y(gpGlobals->viewport) + PAL_Y(lpSpriteToDraw->pos) - lpSpriteToDraw->iLayer;
   int       sh = ((sx % 32) ? 1 : 0);
...
--*/
{
   ...
   PAL_TRACE_LOG("[LAVA][SCENE] rendered scene=%d view=(%d,%d)\n",
      gpGlobals->wNumScene,
