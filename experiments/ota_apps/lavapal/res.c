...
#define RESOURCES struct tagRESOURCES
#define LPRESOURCES struct tagRESOURCES *
...
--*/
{
   ...
   if (gpResources->bLoadFlags & kLoadGlobalData)
   {
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][LOAD] applied music=%d slot=%d\n",
         gpGlobals->wNumMusic, gpGlobals->bCurrentSaveSlot);
   }
   ...
   if (gpResources->bLoadFlags & kLoadScene)
   {
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SCENE] scene=%d map=%d enter=%d teleport=%d events=%d..%d\n",
         gpGlobals->wNumScene,
         gpGlobals->g.rgScene[i].wMapNum,
         gpGlobals->g.rgScene[i].wScriptOnEnter,
         gpGlobals->g.rgScene[i].wScriptOnTeleport,
         gpGlobals->g.rgScene[i].wEventObjectIndex + 1,
         gpGlobals->g.rgScene[i + 1].wEventObjectIndex);
      gpResources->lpMap = PAL_LoadMap(gpGlobals->g.rgScene[i].wMapNum,
         fpMAP, fpGOP);
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SCENE] map=%d mapdec=%d gop=%d\n",
         gpGlobals->g.rgScene[i].wMapNum,
         gpResources->lpMap != NULL ? 1 : -1,
         1);
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SCENE] map=%d events=%d\n",
         gpGlobals->g.rgScene[i].wMapNum,
         gpResources->nEventObject);
      if (gpGlobals->fEnteringScene)
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SCENE] preview deferred scene=%d view=(%d,%d)\n",
            gpGlobals->wNumScene,
   ...
   if (gpResources->bLoadFlags & kLoadPlayerSprite)
   {
      ...
      for (i = 0; i <= (short)gpGlobals->wMaxPartyMemberIndex; i++)
      {
         ...
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][LOAD] player slot=%d role=%d sprite=%d size=%d\n",
            i, wPlayerID, wSpriteNum, l);
      }
      ...
      for (i = 1; i <= gpGlobals->nFollower; i++)
      {
         ...
         //
         wPlayerID = gpGlobals->rgParty[(short)gpGlobals->wMaxPartyMemberIndex+i].wPlayerRole;
         assert(wPlayerID < MAX_PLAYER_ROLES);
         ...
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][PARTY] follower slot=%d role=%d sprite=%d size=%d\n",
            (short)gpGlobals->wMaxPartyMemberIndex + i, wPlayerID, wSpriteNum, l);
      }
   }
