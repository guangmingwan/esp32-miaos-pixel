...
--*/
{
   ...
   else
   {
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEVIS] player pose slot=%d frame=%d defend=%d hp=%d/%d\n",
         wPlayerIndex,
         g_Battle.rgPlayer[wPlayerIndex].wCurrentFrame,
         g_Battle.rgPlayer[wPlayerIndex].fDefending,
         gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[wPlayerIndex].wPlayerRole],
         gpGlobals->g.PlayerRoles.rgwMaxHP[gpGlobals->rgParty[wPlayerIndex].wPlayerRole]);
...
--*/
{
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLE] ui-begin\n");
   ...
   //
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLE] end result=%d\n", g_Battle.BattleResult);
...
--*/
{
   ...
   for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
   {
      ...
      if (l <= 0)
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEVIS] player bad size slot=%d role=%d sprite=%d size=%d\n",
            i, gpGlobals->rgParty[i].wPlayerRole, s, l);
         continue;
      }

      g_Battle.rgPlayer[i].lpSprite = UTIL_calloc(l, 1);

      iRet = PAL_MKFDecompressChunk(g_Battle.rgPlayer[i].lpSprite, l,
         s, gpGlobals->f.fpF);
      if (iRet <= 0)
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEVIS] player sprite fail slot=%d role=%d sprite=%d ret=%d\n",
            i, gpGlobals->rgParty[i].wPlayerRole, s, iRet);
         free(g_Battle.rgPlayer[i].lpSprite);
         g_Battle.rgPlayer[i].lpSprite = NULL;
         continue;
      }
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLE] player slot=%d role=%d sprite=%d pos=(%d,%d)\n",
         i, gpGlobals->rgParty[i].wPlayerRole, s, x, y);
      iFrameCount = PAL_SpriteGetNumFrames(g_Battle.rgPlayer[i].lpSprite);
      lpFrame = PAL_SpriteGetFrame(g_Battle.rgPlayer[i].lpSprite, 0);
      if (lpFrame == NULL)
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEVIS] player frame fail slot=%d role=%d sprite=%d frame=0 frames=%d\n",
            i, gpGlobals->rgParty[i].wPlayerRole, s, iFrameCount);
      }
      else
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEVIS] player ok slot=%d role=%d sprite=%d frames=%d frame0=%dx%d\n",
            i, gpGlobals->rgParty[i].wPlayerRole, s, iFrameCount,
            PAL_RLEGetWidth(lpFrame), PAL_RLEGetHeight(lpFrame));
      }
   }
   ...
   for (i = 0; i < MAX_ENEMIES_IN_TEAM; i++)
   {
...

      {
         ...
         iEnemyID = gpGlobals->g.rgObject[g_Battle.rgEnemy[i].wObjectID].enemy.wEnemyID;
         l = PAL_MKFGetDecompressedSize(iEnemyID, fp);

         if (l <= 0)
         {
            UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEVIS] enemy sprite bad size slot=%d obj=%d enemy=%d size=%d\n",
               i, g_Battle.rgEnemy[i].wObjectID, iEnemyID, l);
            continue;
         }

         g_Battle.rgEnemy[i].lpSprite = UTIL_calloc(l, 1);

         iRet = PAL_MKFDecompressChunk(g_Battle.rgEnemy[i].lpSprite, l,
            iEnemyID, fp);
         if (iRet <= 0)
         {
            UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEVIS] enemy sprite fail slot=%d obj=%d enemy=%d size=%d ret=%d\n",
               i, g_Battle.rgEnemy[i].wObjectID, iEnemyID, l, iRet);
            free(g_Battle.rgEnemy[i].lpSprite);
            g_Battle.rgEnemy[i].lpSprite = NULL;
            continue;
         }
         ...
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLE] enemy slot=%d object=%d enemy=%d pos=(%d,%d)\n",
            i,
            g_Battle.rgEnemy[i].wObjectID,
            iEnemyID,
            x, y);
         iFrameCount = PAL_SpriteGetNumFrames(g_Battle.rgEnemy[i].lpSprite);
         lpFrame = PAL_SpriteGetFrame(g_Battle.rgEnemy[i].lpSprite, 0);
         if (lpFrame == NULL)
         {
            UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEVIS] enemy frame fail slot=%d obj=%d enemy=%d frame=0 frames=%d\n",
               i, g_Battle.rgEnemy[i].wObjectID, iEnemyID, iFrameCount);
         }
         else
         {
            UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEVIS] enemy ok slot=%d obj=%d enemy=%d size=%d frames=%d frame=%dx%d\n",
               i,
               g_Battle.rgEnemy[i].wObjectID,
               iEnemyID,
               l,
               iFrameCount,
               PAL_RLEGetWidth(lpFrame),
               PAL_RLEGetHeight(lpFrame));
         }
      }
   }
...
--*/
{
   ...
   {
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEVIS] bg load fail field=%d ret=%d\n",
         gpGlobals->wNumBattleField, -1);
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEVIS] prepared bg=%d field=%d enemies=%d\n",
      g_Battle.lpBackground != NULL ? 1 : 0,
      gpGlobals->wNumBattleField,
      g_Battle.wMaxEnemyIndex + 1);
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLEVIS] draw bg=%d target=%d count=%d\n",
      g_Battle.lpBackground != NULL ? 1 : 0,
      -1,
      g_Battle.wMaxEnemyIndex + 1);
}
...
--*/
{
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLE] begin team=%d boss=%d\n", wEnemyTeam, fIsBoss ? 1 : 0);
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLE] prepared enemies=%d count=%d field=%d\n",
      g_Battle.wMaxEnemyIndex + 1,
      gpGlobals->wMaxPartyMemberIndex + 1,
      gpGlobals->wNumBattleField);
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLE] prepared actor=%d party_hp=%d/%d,%d/%d,%d/%d\n",
      g_Battle.UI.wCurPlayerIndex,
      gpGlobals->wMaxPartyMemberIndex >= 0 ? gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[0].wPlayerRole] : 0,
      gpGlobals->wMaxPartyMemberIndex >= 0 ? gpGlobals->g.PlayerRoles.rgwMaxHP[gpGlobals->rgParty[0].wPlayerRole] : 0,
      gpGlobals->wMaxPartyMemberIndex >= 1 ? gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[1].wPlayerRole] : 0,
      gpGlobals->wMaxPartyMemberIndex >= 1 ? gpGlobals->g.PlayerRoles.rgwMaxHP[gpGlobals->rgParty[1].wPlayerRole] : 0,
      gpGlobals->wMaxPartyMemberIndex >= 2 ? gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[2].wPlayerRole] : 0,
      gpGlobals->wMaxPartyMemberIndex >= 2 ? gpGlobals->g.PlayerRoles.rgwMaxHP[gpGlobals->rgParty[2].wPlayerRole] : 0);
   for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
   {
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BATTLE] party slot=%d role=%d hp=%d/%d\n",
         i,
         gpGlobals->rgParty[i].wPlayerRole,
         gpGlobals->g.PlayerRoles.rgwHP[gpGlobals->rgParty[i].wPlayerRole],
         gpGlobals->g.PlayerRoles.rgwMaxHP[gpGlobals->rgParty[i].wPlayerRole]);
   }
