...
--*/
{
   ...
   PAL_TRACE_LOG("[LAVA][PLAY] frame=%lu trigger=%d scene=%d entering=%d view=(%d,%d) party=(%d,%d)\n",
      (unsigned long)gpGlobals->dwFrameNum,
      fTrigger,
      gpGlobals->wNumScene,
      gpGlobals->fEnteringScene,
   ...
   if (fTrigger)
   {
      PAL_TRACE_LOG("[LAVA][PLAY] trigger frame=%lu scene=%d entering=%d view=(%d,%d) party=(%d,%d)\n",
         (unsigned long)gpGlobals->dwFrameNum,
         gpGlobals->wNumScene,
         gpGlobals->fEnteringScene,
         PAL_X(gpGlobals->viewport), PAL_Y(gpGlobals->viewport),
         ...
         wEventObjectID++)
      {
         ...
         else if (p->sState > 0 && p->wTriggerMode >= kTriggerTouchNear)
         {
            ...
            {
               UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][TOUCH] object=%d trigger=%d mode=%d state=%d pos=(%d,%d)\n",
                  wEventObjectID, p->wTriggerScript, p->wTriggerMode, p->sState, p->x, p->y);
               //
      ...
      wEventObjectID++)
   {
      ...
      if (p->sState > 0 && p->sVanishTime == 0)
      {
         ...
         if (wScriptEntry != 0)
         {
            PAL_TRACE_LOG("[LAVA][AUTOSCRIPT] object=%d idx=%d state=%d\n",
               wEventObjectID, wScriptEntry, p->sState);
         ...
         abs(p->y - PAL_Y(gpGlobals->viewport) - PAL_Y(gpGlobals->partyoffset)) * 2 <= 12)
      {
         ...
         for (i = 0; i < 4; i++)
         {
            ...
            UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][BLOCK] next=(%d,%d) dir=%d\n",
               x, y, wDir);
...
--*/
{
   ...
   while (TRUE)
   {
      ...
      if (!(gpGlobals->g.rgObject[wObject].item.wFlags & kItemFlagApplyToAll))
      {
         ...
         while (TRUE)
         {
            ...
            UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ITEMUSE] before hp=%d mp=%d role=%d\n",
               gpGlobals->g.PlayerRoles.rgwHP[wPlayer],
               gpGlobals->g.PlayerRoles.rgwMP[wPlayer],
               wPlayer);
      ...
      {
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][ITEMUSE] apply-to-all item=%d\n", wObject);
         //
         // Run the script
         //
          gpGlobals->g.rgObject[wObject].item.wScriptOnUse =
...
--*/
{
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SEARCH] scene=%d party=(%d,%d) dir=%d\n",
      gpGlobals->wNumScene,
   ...
   for (i = 0; i < 13; i++)
   {
         ...
         k < gpGlobals->g.rgScene[gpGlobals->wNumScene].wEventObjectIndex; k++)
      {
         ...
         UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SEARCH] candidate object=%d trigger=%d mode=%d state=%d pos=(%d,%d)\n",
            k + 1, p->wTriggerScript, p->wTriggerMode, p->sState, p->x, p->y);
         ...
         //
          ...
          UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SEARCH] hit object=%d trigger=%d mode=%d checkpoint=%d pos=(%d,%d)\n",
             k + 1, l, p->wTriggerMode, i, p->x, p->y);
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][SEARCH] miss pos=(%d,%d) dir=%d\n",
      PAL_X(gpGlobals->viewport) + PAL_X(gpGlobals->partyoffset),
      PAL_Y(gpGlobals->viewport) + PAL_Y(gpGlobals->partyoffset),
      gpGlobals->wPartyDirection);
}
...
--*/
{
   ...
   //
   PAL_TRACE_LOG("[LAVA][ROUND] frame=%lu key=%lu scene=%d\n",
      (unsigned long)gpGlobals->dwFrameNum,
      (unsigned long)g_InputState.dwKeyPress,
      gpGlobals->wNumScene);
   PAL_TRACE_LOG("[LAVA][FRAMEINPUT] hold=%d framehold=%d press=%d dir=%d pos=(%d,%d)\n",
      0, 0, g_InputState.dwKeyPress, g_InputState.dir,
      PAL_X(gpGlobals->viewport) + PAL_X(gpGlobals->partyoffset),
      PAL_Y(gpGlobals->viewport) + PAL_Y(gpGlobals->partyoffset));
   PAL_GameUpdate(TRUE);
   if (gpGlobals->fEnteringScene)
   ...
   //
   PAL_UpdateParty();
   PAL_TRACE_LOG("[LAVA][MOVE] pos=(%d,%d) view=(%d,%d) dir=%d frame=%d\n",
      PAL_X(gpGlobals->viewport) + PAL_X(gpGlobals->partyoffset),
      PAL_Y(gpGlobals->viewport) + PAL_Y(gpGlobals->partyoffset),
      PAL_X(gpGlobals->viewport), PAL_Y(gpGlobals->viewport),
      gpGlobals->wPartyDirection,
