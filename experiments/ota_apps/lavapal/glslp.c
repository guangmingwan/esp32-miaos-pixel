...
#define LineType enum tagLineType
...
static int token_conform( char *name, LineType type, GLSLP *pGLSLP ) {
    int index = -1;
    int i;
    switch( type ) {
        ...
        case TOKEN_PARAMETER_NAME:
            for( i = 0; i < pGLSLP->uniform_parameters; i++ ) {
...
static char *glslp_pack_texturenames(GLSLP *gGLSLP) {
...
char *glslp_pack_parameters(GLSLP *gGLSLP) {
    memset(glslp_commonbuf,0,sizeof(glslp_commonbuf));
    int i;
...
static enum scale_type string_to_scale_type(char *value) {
...
static enum wrap_mode string_to_wrap_mode(char *value) {
