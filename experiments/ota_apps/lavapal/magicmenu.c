...
static WORD    g_wMagicMenuRole = 0;
...
--*/
{
   ...
   BYTE        bColor;
   WORD        wScript;
   int   iItemsPerLine = 32 / gConfig.dwWordLength;
   int   iItemTextWidth = 8 * gConfig.dwWordLength + 7;
   int   iLinesPerPage = 5 - gConfig.ScreenLayout.ExtraMagicDescLines;
   int   iBoxYOffset = gConfig.ScreenLayout.ExtraMagicDescLines * 16;
   int   iCursorXOffset = gConfig.dwWordLength * 5 / 2;
   int   iPageLineOffset = iLinesPerPage / 2;
   ...
   // Create the box.
   //
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICUI] enter role=%d mp=%d count=%d sel=%d obj=%d\n",
      g_wMagicMenuRole,
      g_wPlayerMP,
      g_iNumMagic,
      g_iCurrentItem,
      (g_iCurrentItem >= 0 && g_iCurrentItem < g_iNumMagic) ? rgMagicItem[g_iCurrentItem].wMagic : 0);
   ...
   if (!gConfig.fIsWIN95)
   {
      ...
      else
      {
         ...
         WCHAR *d = PAL_GetObjectDesc(gpGlobals->lpObjectDesc, rgMagicItem[g_iCurrentItem].wMagic);
   ...
   for (j = 0; j < iLinesPerPage; j++)
   {
      for (k = 0; k < iItemsPerLine; k++)
      {
         ...
         if (i == g_iCurrentItem)
         {
            UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICUI] item i=%d obj=%d cost=%d pos=(%d,%d)\n",
               i, rgMagicItem[i].wMagic, rgMagicItem[i].wMP,
               35 + k * iItemTextWidth, 54 + j * 18 + iBoxYOffset);
...
--*/
{
   ...
   g_wMagicMenuRole = wPlayerRole;

   g_iCurrentItem = 0;
   g_iNumMagic = 0;

   g_wPlayerMP = gpGlobals->g.PlayerRoles.rgwMP[wPlayerRole];
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICDATA] role=%d mp=%d\n", wPlayerRole, g_wPlayerMP);
   ...
   UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICDATA] count=%d\n", g_iNumMagic);
...
--*/
{
   ...
   while (TRUE)
   {
      ...
      UTIL_LogOutput(LOGLEVEL_DEBUG, "[LAVA][MAGICUI] draw role=%d current=%d count=%d\n",
         wPlayerRole, g_iCurrentItem, g_iNumMagic);
