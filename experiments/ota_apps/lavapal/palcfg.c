...
static ConfigItem gConfigItems[PALCFG_ALL_MAX] = {
...
static char *music_types[] = { "RIX", "MP3", "OGG", "OPUS", "RAW" };
static char *cd_types[] = { "NONE", "MP3", "OGG", "OPUS", "RAW" };
...
)
{
	...
	if (*line && *line != '#')
	{
		...
		if ((ptr = strchr(line, '=')) != NULL)
		{
			...
			for (i = 0; i < sizeof(gConfigItems) / sizeof(ConfigItem); i++)
			{
...
{
	int index;
	for (index = PALCFG_ALL_MIN; index < PALCFG_ALL_MAX; index++)
	{
...
)
{
	...
	static SCREENLAYOUT screen_layout = {
		// Equipment Screen
	...
	PALCFG_ITEM i;
	for (i = PALCFG_ALL_MIN; i < PALCFG_ALL_MAX; i++) values[i] = gConfigItems[i].DefaultValue;
...
)
{
	...
	case PALCFG_CD:
		{
			int i;
			for (i = 0; i < sizeof(cd_types) / sizeof(cd_types[0]); i++)
		{
	...
	case PALCFG_MUSIC:
		{
			int i;
			for (i = 0; i < sizeof(music_types) / sizeof(music_types[0]); i++)
		{
