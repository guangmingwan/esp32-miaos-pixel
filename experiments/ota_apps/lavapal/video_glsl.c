...
#pragma pack(pop)
#define GLKMatrix4 union _GLKMatrix4
...
char *readShaderFile(char *filename, GLuint type) {
...
#define SHADER_TYPE(shaderType) (shaderType == GL_VERTEX_SHADER ? "VERTEX" : "FRAGMENT")
    ...
    // Define shader text
    const GLchar *shaderSource = pShaderBuffer;
...
GLuint compileProgram(char* vtx, char* frag,int is_source) {
    GLuint programId = 0;
...
int VIDEO_RenderTexture(SDL_Renderer * renderer, SDL_Texture * texture, SDL_Rect * srcrect, SDL_Rect * dstrect, int pass)
{
    GLint oldProgramId;
...
#if SUPPORT_PARAMETER_UNIFORM
    for( i=0; i < gGLSLP.uniform_parameters; i++ ) {
