/*
 * minesweeper — OTA app entry point
 * Thin FreeRTOS wrapper around minesweeper_main_impl.
 */
...
extern int minesweeper_main_impl(int argc, char *argv[]);
...
static void ui_task(void *arg) {
    char *argv[] = {"minesweeper"};
    (void)arg;

    minesweeper_main_impl(1, argv);
    switch_to_launcher();
    esp_restart();
...
void app_main(void) {
    ...
    xTaskCreatePinnedToCore(ui_task, "minesweeper_ui", 16384, NULL, 5, NULL, 1);
}
