...
static unsigned char cartridge_sram[0x10000];
static unsigned int cartridge_sram_start;
static size_t cartridge_sram_size;
static int cartridge_sram_dirty;

void gwenesis_bus_configure_sram(unsigned int start, size_t size) {
  cartridge_sram_start = start;
  cartridge_sram_size = size <= sizeof(cartridge_sram) ? size : 0;
  cartridge_sram_dirty = 0;
  memset(cartridge_sram, 0xff, sizeof(cartridge_sram));
}
unsigned char *gwenesis_bus_sram_data(void) { return cartridge_sram; }
size_t gwenesis_bus_sram_size(void) { return cartridge_sram_size; }
int gwenesis_bus_sram_dirty(void) { return cartridge_sram_dirty; }
void gwenesis_bus_sram_mark_clean(void) { cartridge_sram_dirty = 0; }
...
unsigned int gwenesis_bus_map_address(unsigned int address) {
  if (cartridge_sram_size != 0 && address >= cartridge_sram_start &&
      address - cartridge_sram_start < cartridge_sram_size)
    return SRAM_ADDR;
  // Mask address page
...
static inline unsigned int gwenesis_bus_read_memory_8(unsigned int address) {
 bus_log(__FUNCTION__,"read8  %x", address);
  ...
  case SRAM_ADDR:
    return cartridge_sram[address - cartridge_sram_start];
...
static inline unsigned int gwenesis_bus_read_memory_16(unsigned int address) {
  ...
  case SRAM_ADDR:
    return (cartridge_sram[address - cartridge_sram_start] << 8) |
           cartridge_sram[address - cartridge_sram_start + 1];
...
static inline void gwenesis_bus_write_memory_8(unsigned int address,
  ...
  case SRAM_ADDR:
    cartridge_sram[address - cartridge_sram_start] = value;
    cartridge_sram_dirty = 1;
    return;
...
static inline void gwenesis_bus_write_memory_16(unsigned int address,
  ...
  case SRAM_ADDR:
    cartridge_sram[address - cartridge_sram_start] = value >> 8;
    cartridge_sram[address - cartridge_sram_start + 1] = value;
    cartridge_sram_dirty = 1;
    return;
