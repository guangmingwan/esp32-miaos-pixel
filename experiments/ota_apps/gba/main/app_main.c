...
static esp_err_t switch_to_launcher(void) {
    ...
    if (error == ESP_OK) error = esp_partition_write(partition, 0, &entries[0], sizeof(entries[0]));
    if (error == ESP_OK) error = esp_partition_write(partition, 4096, &entries[1], sizeof(entries[1]));
    return error;
}
...
static void emulator_task(void *argument) {
    ...
    ESP_LOGE("gba_app", "launcher return failed: %s", esp_err_to_name(error));
    vTaskDelete(NULL);
}

void app_main(void) {
    ESP_ERROR_CHECK(host_platform_init());
    BaseType_t created = xTaskCreatePinnedToCore(emulator_task, "gba_emu", 49152, NULL, 5, NULL, 1);
    if (created != pdPASS) ESP_LOGE("gba_app", "emulator task allocation failed");
}
