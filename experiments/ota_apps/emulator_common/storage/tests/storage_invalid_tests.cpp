...
int main() {
    const MiaStorageTarget *gba = nullptr;
    const MiaStorageTarget *doom = nullptr;
    require_status(mia_storage_target_find(&mia_storage_generated_targets, "gba", &gba), MIA_STORAGE_OK);
    ...
    std::ofstream(root / "roms" / "gba" / "bad.zip").put('z');

    MiaStorageContext context{root.c_str()};
    require_status(mia_storage_validate_requirements(&context, gba), MIA_STORAGE_ERR_MISSING_REQUIRED_FILE);
    ...
    require_status(mia_storage_picker_select(&context, gba, "bad.zip", &result), MIA_STORAGE_ERR_UNSUPPORTED_ARCHIVE);
