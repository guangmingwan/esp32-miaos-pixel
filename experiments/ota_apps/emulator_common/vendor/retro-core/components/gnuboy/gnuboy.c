...
size_t gnuboy_sram_size(void)
{
	if (!cart.has_battery || !cart.ramsize)
		return 0;
	return (size_t)cart.ramsize * 8192u + (cart.has_rtc ? 48u : 0u);
}

int gnuboy_export_sram(void *data, size_t capacity)
{
	const size_t size = gnuboy_sram_size();
	if (!data || size == 0 || capacity < size)
		return -1;
	byte *dest = data;
	for (int i = 0; i < cart.ramsize; ++i)
		memcpy(dest + (size_t)i * 8192u, cart.rambanks[i], 8192u);
	if (cart.has_rtc)
	{
		uint64_t rt = RTC_BASE + cart.rtc.s + (cart.rtc.m * 60) + (cart.rtc.h * 3600) + (cart.rtc.d * 86400);
		uint32_t rtc_buf[12] = {cart.rtc.s, cart.rtc.m, cart.rtc.h, cart.rtc.d, cart.rtc.flags,
			cart.rtc.regs[0], cart.rtc.regs[1], cart.rtc.regs[2], cart.rtc.regs[3], cart.rtc.regs[4],
			(uint32_t)rt, (uint32_t)(rt >> 32)};
		memcpy(dest + (size_t)cart.ramsize * 8192u, rtc_buf, sizeof(rtc_buf));
	}
	return (int)size;
}

int gnuboy_import_sram(const void *data, size_t size)
{
	const size_t expected = gnuboy_sram_size();
	if (!data || expected == 0 || size != expected)
		return -1;
	const byte *source = data;
	for (int i = 0; i < cart.ramsize; ++i)
		memcpy(cart.rambanks[i], source + (size_t)i * 8192u, 8192u);
	if (cart.has_rtc)
	{
		uint32_t rtc_buf[12];
		memcpy(rtc_buf, source + (size_t)cart.ramsize * 8192u, sizeof(rtc_buf));
		cart.rtc = (gb_rtc_t){.s = rtc_buf[0], .m = rtc_buf[1], .h = rtc_buf[2], .d = rtc_buf[3],
			.flags = rtc_buf[4], .regs = {rtc_buf[5], rtc_buf[6], rtc_buf[7], rtc_buf[8], rtc_buf[9]}};
	}
	cart.sram_dirty = 0;
...
void gnuboy_mark_sram_saved(void)
{
	cart.sram_dirty = 0;
	cart.sram_saved = (1 << cart.ramsize) - 1;
}
