...
public:
   ...
   int Size(void);
    ...
    // MiaOS target adaptation: expose EEPROM bytes for atomic host persistence.
    int ExportTo(char *data,int count){ int size=__min(count,Size()); memcpy(data,romdata,size); return size; };
