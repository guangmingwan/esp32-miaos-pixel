...
static void initVcpTask(void *readyTask) {
  Serial.setRxBufferSize(8192);
  Serial.setTxBufferSize(8192);
  Serial.begin(115200);
  xTaskNotifyGive(static_cast<TaskHandle_t>(readyTask));
  vTaskDelete(nullptr);
}
...
static void drawSdFlashProgress(size_t completed, size_t total, void *context) {
  ...
  lavaDrawText(boxX + 8, lavaTextYCentered(boxY + 2, 20), miaTr("Loading"),
               LAVA_BLACK, LAVA_YELLOW);
...
static void drawLauncher() {
  ...
  for (uint8_t tab = 0; tab < tabCount; ++tab) {
    ...
    if (drawEnd > 0 && drawX < LAVA_SCREEN_W) {
      ...
      lavaDrawText(drawX + 6, lavaTextYCentered(28, 20), tabName, fg, bg);
    }
  ...
  for (uint8_t i = firstApp; i < visibleEnd; ++i) {
    ...
    const int16_t itemTextY = lavaTextYCentered(y, appRowHeight);
  ...
  if (g_bootLoaderHintVisible) {
    ...
    lavaDrawText(30, lavaTextYCentered(44, 20), miaTr("Boot Loader"), LAVA_BLACK,
                 LAVA_YELLOW);
  ...
  if (g_sdActionMenuVisible) {
    ...
    lavaDrawText(34, lavaTextYCentered(54, 20),
    ...
    for (uint8_t i = 0; i < 2; ++i) {
      ...
      lavaDrawText(52, lavaTextYCentered(menuY, 16), miaTr(MENU_ITEMS[i]), fg, bg);
    }
  ...
  if (g_otaExportConfirmVisible) {
    ...
    lavaDrawText(boxX + 8, lavaTextYCentered(boxY + 2, 18), miaTr("Export OTA to SD"), LAVA_BLACK,
                 LAVA_YELLOW);
    char catLine[48];
  ...
  if (g_otaExportResultVisible) {
    ...
    lavaDrawText(boxX + 8, lavaTextYCentered(boxY + 2, 18),
...
void setup() {
  ...
  xTaskCreatePinnedToCore(initVcpTask, "vcp_init", 4096, xTaskGetCurrentTaskHandle(), 3,
                          &vcpInitTask, 1);
  ulTaskNotifyTake(pdTRUE, portMAX_DELAY);

  Serial.println();
  Serial.println("ESP32-S3 Retro-Pixel launcher (ILI9342 320x240)");
  Serial.printf("CPU frequency: %uMHz set=%s\n", ESP.getCpuFreqMHz(),
                cpuAt240Mhz ? "OK" : "FAILED");
...
void loop() {
  ...
  if (serialFileServiceTakeEnterRequest()) {
    launcherTrace("[vcp-control] host requested VCP File Transfer");
